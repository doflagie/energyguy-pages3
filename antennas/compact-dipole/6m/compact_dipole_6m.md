# 6m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-6M-001 Rev A
**Equipment:** 6m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 6m Amateur Radio |
| **Center frequency** | 52.000 MHz |
| **Band edge (low)** | 50.000 MHz |
| **Band edge (high)** | 54.000 MHz |
...
|-----------|-------|-------|
| **Full half-dipole length** | 1.371 m (4.50 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.823 m (2.70 ft) | With center loading coil |
| **Compaction factor** | 60% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.25 m | Fully rolled on card |
| **Deployed span** | 1.645 m (5.40 ft) | Both arms extended |
...
| **Efficiency** | 80–85% (high-Q coil at design frequency) |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 0.40 μH | Per arm (center-loaded) |
| **Number of turns** | 3 | Close-wound, no gaps |
| **Wire gauge** | AWG 28 | 0.0126" diameter |
| **Total wire length** | 0.13 m (0.4 feet) | Both coils |
| **Form material** | 3/8" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.280" (0.71 cm) | Rigid substrate |
| **Coil length** | 0.038" (0.10 cm) | Close-wound on form |
| **Estimated Q factor** | 250 | High-Q design, low loss |
...
- Cut to length: 0.04 + 1/4" for margin = 0.29"
...
✓ Inductance test: 0.36–0.43 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 0.40 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 4.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 52.000 MHz |
| **Trap inductance** | Calculated from LC resonance | 2.342 μH |
...
**Verification:** 2.342 μH || 4.0 pF should resonate at 52.000 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 52.000 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 21 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 1.65 m (5.4 feet)
...
**Required equipment:**
- SWR/power meter (capable of 50.00–54.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 52.000 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 50.000 MHz
   - Record SWR at band center: __________ @ 52.000 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 54.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_6m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 52.000 MHz
...
The following diagrams are located in this band folder (`6m/figures/`):

1. **6m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **6m_coil_detail.png** — Loading coil cross-section, winding detail
3. **6m_trap_schematic.png** — Trap resonator circuit, L/C values
