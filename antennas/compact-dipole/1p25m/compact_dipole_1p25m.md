# 1.25m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-1P25M-001 Rev A
**Equipment:** 1.25m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 1.25m Amateur Radio |
| **Center frequency** | 222.500 MHz |
| **Band edge (low)** | 220.000 MHz |
| **Band edge (high)** | 225.000 MHz |
| **Band width** | 5.000 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 0.320 m (1.05 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.304 m (1.00 ft) | With center loading coil |
| **Compaction factor** | 95% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.09 m | Fully rolled on card |
| **Deployed span** | 0.609 m (2.00 ft) | Both arms extended |
...
| **Bandwidth (2:1 SWR)** | 1–2 kHz (estimated) |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 0.02 μH | Per arm (center-loaded) |
| **Number of turns** | 1 | Close-wound, no gaps |
| **Wire gauge** | AWG 30 | 0.0100" diameter |
| **Total wire length** | 0.03 m (0.1 feet) | Both coils |
| **Form material** | 1/4" FR4 | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.200" (0.51 cm) | Rigid substrate |
| **Coil length** | 0.010" (0.03 cm) | Close-wound on form |
...
- Obtain rigid PVC pipe, 1/4" FR4 OD
- Cut to length: 0.01 + 1/4" for margin = 0.26"
...
5. **Target:** 1 turns, close-wound, no gaps
...
✓ Inductance test: 0.02–0.02 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 0.02 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 2.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 222.500 MHz |
| **Trap inductance** | Calculated from LC resonance | 0.256 μH |
...
**Verification:** 0.256 μH || 2.0 pF should resonate at 222.500 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 222.500 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 8 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 0.61 m (2.0 feet)
...
**Required equipment:**
- SWR/power meter (capable of 220.00–225.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 222.500 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 220.000 MHz
   - Record SWR at band center: __________ @ 222.500 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 225.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_1p25m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 222.500 MHz
...
The following diagrams are located in this band folder (`1p25m/figures/`):

1. **1p25m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **1p25m_coil_detail.png** — Loading coil cross-section, winding detail
3. **1p25m_trap_schematic.png** — Trap resonator circuit, L/C values
