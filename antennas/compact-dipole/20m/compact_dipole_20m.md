# 20m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-20M-001 Rev A
**Equipment:** 20m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 20m Amateur Radio |
| **Center frequency** | 14.175 MHz |
| **Band edge (low)** | 14.000 MHz |
| **Band edge (high)** | 14.350 MHz |
| **Band width** | 0.350 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 5.030 m (16.50 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 2.515 m (8.25 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.75 m | Fully rolled on card |
| **Deployed span** | 5.030 m (16.50 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 5.32 μH | Per arm (center-loaded) |
| **Number of turns** | 11 | Close-wound, no gaps |
| **Wire gauge** | AWG 24 | 0.0201" diameter |
| **Total wire length** | 0.92 m (3.0 feet) | Both coils |
| **Form material** | 3/4" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.525" (1.33 cm) | Rigid substrate |
| **Coil length** | 0.221" (0.56 cm) | Close-wound on form |
...
- Cut to length: 0.22 + 1/4" for margin = 0.47"
...
✓ Inductance test: 4.79–5.85 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 5.32 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 10.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 14.175 MHz |
| **Trap inductance** | Calculated from LC resonance | 12.606 μH |
...
**Verification:** 12.606 μH || 10.0 pF should resonate at 14.175 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 14.175 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 63 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 5.03 m (16.5 feet)
...
**Required equipment:**
- SWR/power meter (capable of 14.00–14.35 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 14.175 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 14.000 MHz
   - Record SWR at band center: __________ @ 14.175 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 14.350 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_20m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 14.175 MHz
...
The following diagrams are located in this band folder (`20m/figures/`):

1. **20m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **20m_coil_detail.png** — Loading coil cross-section, winding detail
3. **20m_trap_schematic.png** — Trap resonator circuit, L/C values
