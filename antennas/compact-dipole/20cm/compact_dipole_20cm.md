# 20cm COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-20CM-001 Rev A
**Equipment:** 20cm COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 20cm Amateur Radio |
| **Center frequency** | 1270.000 MHz |
| **Band edge (low)** | 1240.000 MHz |
| **Band edge (high)** | 1300.000 MHz |
| **Band width** | 60.000 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 0.056 m (0.18 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.053 m (0.17 ft) | With center loading coil |
| **Compaction factor** | 95% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.02 m | Fully rolled on card |
| **Deployed span** | 0.107 m (0.35 ft) | Both arms extended |
...
| **Efficiency** | 75–82% (high-Q coil at design frequency) |
| **Bandwidth (2:1 SWR)** | 12–30 kHz (estimated) |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 0.00 μH | Per arm (center-loaded) |
| **Number of turns** | 0 | Close-wound, no gaps |
| **Wire gauge** | AWG 30 | 0.0100" diameter |
| **Total wire length** | 0.00 m (0.0 feet) | Both coils |
| **Form material** | 1/8" ceramic | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.125" (0.32 cm) | Rigid substrate |
| **Coil length** | 0.000" (0.00 cm) | Close-wound on form |
| **Estimated Q factor** | 300 | High-Q design, low loss |
| **Coil DC resistance** | 0.0 mΩ | (estimate, AWG 30) |

### 2.2 Fabrication Procedure

#### Step 1: Form Preparation
- Obtain rigid PVC pipe, 1/8" ceramic OD
- Cut to length: 0.00 + 1/4" for margin = 0.25"
- Deburr ends with 120-grit sandpaper
- Inspect for cracks or defects

#### Step 2: Coil Winding
1. Secure form in lathe or hand-wind jig
2. Feed magnet wire (AWG 30) through form center
3. Wind tight helical coil, pitch = wire diameter (0.255 mm)
4. Maintain tension for uniform spacing
5. **Target:** 0 turns, close-wound, no gaps
6. Mark first and last turn with marker pen

#### Step 3: Continuity & Inductance Verification
- Measure DC resistance end-to-end
  - Expected: 0.0m × 0.026Ω/m ≈ 0.00 mΩ
  - Acceptable range: ±20% of expected value
- Measure inductance with LCR meter or dip meter
  - Expected: 0.00 μH
  - Acceptable: 0.00–0.00 μH (±10%)
...
✓ Inductance test: 0.00–0.00 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 0.00 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 0.8 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 1270.000 MHz |
| **Trap inductance** | Calculated from LC resonance | 0.020 μH |
...
**Verification:** 0.020 μH || 0.8 pF should resonate at 1270.000 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 1270.000 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 1 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 0.11 m (0.3 feet)
...
**Required equipment:**
- SWR/power meter (capable of 1240.00–1300.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 1270.000 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 1240.000 MHz
   - Record SWR at band center: __________ @ 1270.000 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 1300.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_20cm.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 1270.000 MHz
...
- Coil wire AWG 30: Limited by heating
...
The following diagrams are located in this band folder (`20cm/figures/`):

1. **20cm_antenna_layout.png** — Scale deployment drawing with dimensions
2. **20cm_coil_detail.png** — Loading coil cross-section, winding detail
3. **20cm_trap_schematic.png** — Trap resonator circuit, L/C values
