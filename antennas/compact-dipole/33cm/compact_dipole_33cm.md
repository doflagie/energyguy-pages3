# 33cm COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-33CM-001 Rev A
**Equipment:** 33cm COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 33cm Amateur Radio |
| **Center frequency** | 915.000 MHz |
| **Band edge (low)** | 902.000 MHz |
| **Band edge (high)** | 928.000 MHz |
| **Band width** | 26.000 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 0.078 m (0.26 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.074 m (0.24 ft) | With center loading coil |
| **Compaction factor** | 95% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.02 m | Fully rolled on card |
| **Deployed span** | 0.148 m (0.49 ft) | Both arms extended |
...
| **Bandwidth (2:1 SWR)** | 5–13 kHz (estimated) |
...
| **Capacitance** | Variable trimmer, parallel | 1.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 915.000 MHz |
| **Trap inductance** | Calculated from LC resonance | 0.030 μH |
...
**Verification:** 0.030 μH || 1.0 pF should resonate at 915.000 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 915.000 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 2 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 0.15 m (0.5 feet)
...
**Required equipment:**
- SWR/power meter (capable of 902.00–928.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 915.000 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 902.000 MHz
   - Record SWR at band center: __________ @ 915.000 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 928.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_33cm.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 915.000 MHz
...
The following diagrams are located in this band folder (`33cm/figures/`):

1. **33cm_antenna_layout.png** — Scale deployment drawing with dimensions
2. **33cm_coil_detail.png** — Loading coil cross-section, winding detail
3. **33cm_trap_schematic.png** — Trap resonator circuit, L/C values
