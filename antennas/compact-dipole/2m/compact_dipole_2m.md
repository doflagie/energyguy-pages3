# 2m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-2M-001 Rev A
**Equipment:** 2m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 2m Amateur Radio |
| **Center frequency** | 146.000 MHz |
| **Band edge (low)** | 144.000 MHz |
| **Band edge (high)** | 148.000 MHz |
| **Band width** | 4.000 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 0.488 m (1.60 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.293 m (0.96 ft) | With center loading coil |
| **Compaction factor** | 60% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.09 m | Fully rolled on card |
| **Deployed span** | 0.586 m (1.92 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 0.05 μH | Per arm (center-loaded) |
| **Number of turns** | 1 | Close-wound, no gaps |
| **Wire gauge** | AWG 28 | 0.0126" diameter |
| **Total wire length** | 0.04 m (0.1 feet) | Both coils |
| **Form material** | 3/8" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.280" (0.71 cm) | Rigid substrate |
| **Coil length** | 0.013" (0.03 cm) | Close-wound on form |
| **Estimated Q factor** | 300 | High-Q design, low loss |
| **Coil DC resistance** | 0.0 mΩ | (estimate, AWG 28) |

### 2.2 Fabrication Procedure

#### Step 1: Form Preparation
- Obtain rigid PVC pipe, 3/8" PVC OD
...
2. Feed magnet wire (AWG 28) through form center
3. Wind tight helical coil, pitch = wire diameter (0.321 mm)
...
✓ Inductance test: 0.05–0.06 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 0.05 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 3.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 146.000 MHz |
| **Trap inductance** | Calculated from LC resonance | 0.396 μH |
...
**Verification:** 0.396 μH || 3.0 pF should resonate at 146.000 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 146.000 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 7 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 0.59 m (1.9 feet)
...
**Required equipment:**
- SWR/power meter (capable of 144.00–148.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 146.000 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 144.000 MHz
   - Record SWR at band center: __________ @ 146.000 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 148.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_2m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 146.000 MHz
...
- Coil wire AWG 28: Limited by heating
...
The following diagrams are located in this band folder (`2m/figures/`):

1. **2m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **2m_coil_detail.png** — Loading coil cross-section, winding detail
3. **2m_trap_schematic.png** — Trap resonator circuit, L/C values
