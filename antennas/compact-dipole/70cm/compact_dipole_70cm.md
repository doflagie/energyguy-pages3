# 70cm COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-70CM-001 Rev A
**Equipment:** 70cm COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 70cm Amateur Radio |
| **Center frequency** | 435.000 MHz |
| **Band edge (low)** | 420.000 MHz |
| **Band edge (high)** | 450.000 MHz |
| **Band width** | 30.000 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 0.164 m (0.54 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 0.156 m (0.51 ft) | With center loading coil |
| **Compaction factor** | 95% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.05 m | Fully rolled on card |
| **Deployed span** | 0.311 m (1.02 ft) | Both arms extended |
...
| **Bandwidth (2:1 SWR)** | 6–15 kHz (estimated) |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 0.01 μH | Per arm (center-loaded) |
| **Number of turns** | 0 | Close-wound, no gaps |
| **Wire gauge** | AWG 30 | 0.0100" diameter |
| **Total wire length** | 0.00 m (0.0 feet) | Both coils |
| **Form material** | 1/4" ceramic | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.180" (0.46 cm) | Rigid substrate |
...
- Obtain rigid PVC pipe, 1/4" ceramic OD
...
✓ Inductance test: 0.01–0.01 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 0.01 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 1.5 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 435.000 MHz |
| **Trap inductance** | Calculated from LC resonance | 0.089 μH |
...
**Verification:** 0.089 μH || 1.5 pF should resonate at 435.000 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 435.000 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 4 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 0.31 m (1.0 feet)
...
**Required equipment:**
- SWR/power meter (capable of 420.00–450.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 435.000 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 420.000 MHz
   - Record SWR at band center: __________ @ 435.000 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 450.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_70cm.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 435.000 MHz
...
The following diagrams are located in this band folder (`70cm/figures/`):

1. **70cm_antenna_layout.png** — Scale deployment drawing with dimensions
2. **70cm_coil_detail.png** — Loading coil cross-section, winding detail
3. **70cm_trap_schematic.png** — Trap resonator circuit, L/C values
