# 12m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-12M-001 Rev A
**Equipment:** 12m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 12m Amateur Radio |
| **Center frequency** | 24.940 MHz |
| **Band edge (low)** | 24.890 MHz |
| **Band edge (high)** | 24.990 MHz |
...
|-----------|-------|-------|
| **Full half-dipole length** | 2.859 m (9.38 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 1.429 m (4.69 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.43 m | Fully rolled on card |
| **Deployed span** | 2.859 m (9.38 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 1.72 μH | Per arm (center-loaded) |
| **Number of turns** | 7 | Close-wound, no gaps |
| **Wire gauge** | AWG 26 | 0.0159" diameter |
| **Total wire length** | 0.45 m (1.5 feet) | Both coils |
| **Form material** | 1/2" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.400" (1.02 cm) | Rigid substrate |
| **Coil length** | 0.112" (0.28 cm) | Close-wound on form |
...
- Cut to length: 0.11 + 1/4" for margin = 0.36"
...
5. **Target:** 7 turns, close-wound, no gaps
...
✓ Inductance test: 1.55–1.89 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 1.72 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 6.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 24.940 MHz |
| **Trap inductance** | Calculated from LC resonance | 6.787 μH |
...
**Verification:** 6.787 μH || 6.0 pF should resonate at 24.940 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 24.940 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 36 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 2.86 m (9.4 feet)
...
**Required equipment:**
- SWR/power meter (capable of 24.89–24.99 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 24.940 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 24.890 MHz
   - Record SWR at band center: __________ @ 24.940 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 24.990 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_12m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 24.940 MHz
...
The following diagrams are located in this band folder (`12m/figures/`):

1. **12m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **12m_coil_detail.png** — Loading coil cross-section, winding detail
3. **12m_trap_schematic.png** — Trap resonator circuit, L/C values
