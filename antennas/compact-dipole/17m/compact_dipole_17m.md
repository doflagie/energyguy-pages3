# 17m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-17M-001 Rev A
**Equipment:** 17m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 17m Amateur Radio |
| **Center frequency** | 18.118 MHz |
| **Band edge (low)** | 18.068 MHz |
| **Band edge (high)** | 18.168 MHz |
| **Band width** | 0.100 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 3.935 m (12.91 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 1.968 m (6.46 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.59 m | Fully rolled on card |
| **Deployed span** | 3.935 m (12.91 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 3.25 μH | Per arm (center-loaded) |
| **Number of turns** | 10 | Close-wound, no gaps |
| **Wire gauge** | AWG 24 | 0.0201" diameter |
| **Total wire length** | 0.64 m (2.1 feet) | Both coils |
| **Form material** | 1/2" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.400" (1.02 cm) | Rigid substrate |
| **Coil length** | 0.201" (0.51 cm) | Close-wound on form |
...
- Cut to length: 0.20 + 1/4" for margin = 0.45"
...
✓ Inductance test: 2.93–3.58 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 3.25 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 8.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 18.118 MHz |
| **Trap inductance** | Calculated from LC resonance | 9.646 μH |
...
**Verification:** 9.646 μH || 8.0 pF should resonate at 18.118 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 18.118 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 49 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 3.94 m (12.9 feet)
...
**Required equipment:**
- SWR/power meter (capable of 18.07–18.17 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 18.118 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 18.068 MHz
   - Record SWR at band center: __________ @ 18.118 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 18.168 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_17m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 18.118 MHz
...
The following diagrams are located in this band folder (`17m/figures/`):

1. **17m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **17m_coil_detail.png** — Loading coil cross-section, winding detail
3. **17m_trap_schematic.png** — Trap resonator circuit, L/C values
