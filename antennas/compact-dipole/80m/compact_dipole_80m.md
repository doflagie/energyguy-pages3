# 80m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-80M-001 Rev A
**Equipment:** 80m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 80m Amateur Radio |
| **Center frequency** | 3.750 MHz |
| **Band edge (low)** | 3.500 MHz |
| **Band edge (high)** | 4.000 MHz |
| **Band width** | 0.500 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 19.013 m (62.38 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 9.507 m (31.19 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~2.85 m | Fully rolled on card |
| **Deployed span** | 19.013 m (62.38 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 75.98 μH | Per arm (center-loaded) |
| **Number of turns** | 65 | Close-wound, no gaps |
| **Wire gauge** | AWG 22 | 0.0253" diameter |
| **Total wire length** | 6.48 m (21.3 feet) | Both coils |
| **Form material** | 1" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.625" (1.59 cm) | Rigid substrate |
| **Coil length** | 1.648" (4.18 cm) | Close-wound on form |
| **Estimated Q factor** | 200 | High-Q design, low loss |
| **Coil DC resistance** | 0.1 mΩ | (estimate, AWG 22) |

### 2.2 Fabrication Procedure

#### Step 1: Form Preparation
- Obtain rigid PVC pipe, 1" PVC OD
- Cut to length: 1.65 + 1/4" for margin = 1.90"
...
✓ Inductance test: 68.38–83.58 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 75.98 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 25.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 3.750 MHz |
| **Trap inductance** | Calculated from LC resonance | 72.051 μH |
...
**Verification:** 72.051 μH || 25.0 pF should resonate at 3.750 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 3.750 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 238 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 19.01 m (62.4 feet)
...
**Required equipment:**
- SWR/power meter (capable of 3.50–4.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 3.750 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 3.500 MHz
   - Record SWR at band center: __________ @ 3.750 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 4.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_80m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 3.750 MHz
...
The following diagrams are located in this band folder (`80m/figures/`):

1. **80m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **80m_coil_detail.png** — Loading coil cross-section, winding detail
3. **80m_trap_schematic.png** — Trap resonator circuit, L/C values
