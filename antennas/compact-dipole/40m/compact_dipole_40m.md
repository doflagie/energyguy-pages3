# 40m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-40M-001 Rev A
**Equipment:** 40m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 40m Amateur Radio |
| **Center frequency** | 7.150 MHz |
| **Band edge (low)** | 7.000 MHz |
| **Band edge (high)** | 7.300 MHz |
| **Band width** | 0.300 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 9.972 m (32.72 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 4.986 m (16.36 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~1.50 m | Fully rolled on card |
| **Deployed span** | 9.972 m (32.72 ft) | Both arms extended |
...
✓ Inductance test: 18.81–22.99 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 20.90 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 15.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 7.150 MHz |
| **Trap inductance** | Calculated from LC resonance | 33.032 μH |
...
**Verification:** 33.032 μH || 15.0 pF should resonate at 7.150 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 7.150 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 125 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 9.97 m (32.7 feet)
...
**Required equipment:**
- SWR/power meter (capable of 7.00–7.30 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 7.150 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 7.000 MHz
   - Record SWR at band center: __________ @ 7.150 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 7.300 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_40m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 7.150 MHz
...
The following diagrams are located in this band folder (`40m/figures/`):

1. **40m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **40m_coil_detail.png** — Loading coil cross-section, winding detail
3. **40m_trap_schematic.png** — Trap resonator circuit, L/C values
