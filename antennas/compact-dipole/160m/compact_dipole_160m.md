# 160m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-160M-001 Rev A
**Equipment:** 160m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 160m Amateur Radio |
| **Center frequency** | 1.900 MHz |
| **Band edge (low)** | 1.800 MHz |
| **Band edge (high)** | 2.000 MHz |
| **Band width** | 0.200 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 37.526 m (123.12 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 18.763 m (61.56 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~5.63 m | Fully rolled on card |
| **Deployed span** | 37.526 m (123.12 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 295.97 μH | Per arm (center-loaded) |
| **Number of turns** | 156 | Close-wound, no gaps |
| **Wire gauge** | AWG 22 | 0.0253" diameter |
| **Total wire length** | 18.67 m (61.3 feet) | Both coils |
| **Form material** | 1.5" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.750" (1.91 cm) | Rigid substrate |
| **Coil length** | 3.954" (10.04 cm) | Close-wound on form |
| **Estimated Q factor** | 200 | High-Q design, low loss |
| **Coil DC resistance** | 0.3 mΩ | (estimate, AWG 22) |

### 2.2 Fabrication Procedure

#### Step 1: Form Preparation
- Obtain rigid PVC pipe, 1.5" PVC OD
- Cut to length: 3.95 + 1/4" for margin = 4.20"
- Deburr ends with 120-grit sandpaper
- Inspect for cracks or defects

#### Step 2: Coil Winding
1. Secure form in lathe or hand-wind jig
2. Feed magnet wire (AWG 22) through form center
3. Wind tight helical coil, pitch = wire diameter (0.644 mm)
4. Maintain tension for uniform spacing
5. **Target:** 156 turns, close-wound, no gaps
6. Mark first and last turn with marker pen

#### Step 3: Continuity & Inductance Verification
- Measure DC resistance end-to-end
  - Expected: 18.7m × 0.016Ω/m ≈ 0.30 mΩ
  - Acceptable range: ±20% of expected value
- Measure inductance with LCR meter or dip meter
  - Expected: 295.97 μH
  - Acceptable: 266.38–325.57 μH (±10%)
...
✓ Inductance test: 266.38–325.57 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 295.97 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 30.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 1.900 MHz |
| **Trap inductance** | Calculated from LC resonance | 233.890 μH |
...
**Verification:** 233.890 μH || 30.0 pF should resonate at 1.900 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 1.900 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 469 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 37.53 m (123.1 feet)
...
**Required equipment:**
- SWR/power meter (capable of 1.80–2.00 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 1.900 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 1.800 MHz
   - Record SWR at band center: __________ @ 1.900 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 2.000 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_160m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 1.900 MHz
...
- Coil wire AWG 22: Limited by heating
...
The following diagrams are located in this band folder (`160m/figures/`):

1. **160m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **160m_coil_detail.png** — Loading coil cross-section, winding detail
3. **160m_trap_schematic.png** — Trap resonator circuit, L/C values
