# 15m COMPACT DIPOLE ANTENNA
## TECHNICAL DATA SHEET

**Document Number:** TM-15M-001 Rev A
**Equipment:** 15m COMPACT DIPOLE ANTENNA
...
| Parameter | Value |
|-----------|-------|
| **Band** | 15m Amateur Radio |
| **Center frequency** | 21.225 MHz |
| **Band edge (low)** | 21.000 MHz |
| **Band edge (high)** | 21.450 MHz |
| **Band width** | 0.450 MHz |

### 1.2 Physical Dimensions

| Parameter | Value | Notes |
|-----------|-------|-------|
| **Full half-dipole length** | 3.359 m (11.02 ft) | Unloaded, λ/4 @ VF=0.95 |
| **Compact half-dipole length** | 1.680 m (5.51 ft) | With center loading coil |
| **Compaction factor** | 50% | Size reduction vs full-size |
| **Roll-up diameter** | ~0.50 m | Fully rolled on card |
| **Deployed span** | 3.359 m (11.02 ft) | Both arms extended |
...
| Parameter | Value | Notes |
|-----------|-------|-------|
| **Target inductance** | 2.37 μH | Per arm (center-loaded) |
| **Number of turns** | 8 | Close-wound, no gaps |
| **Wire gauge** | AWG 26 | 0.0159" diameter |
| **Total wire length** | 0.51 m (1.7 feet) | Both coils |
| **Form material** | 1/2" PVC | Schedule 40 PVC or equivalent |
| **Form inside radius** | 0.400" (1.02 cm) | Rigid substrate |
| **Coil length** | 0.128" (0.32 cm) | Close-wound on form |
...
- Cut to length: 0.13 + 1/4" for margin = 0.38"
...
✓ Inductance test: 2.13–2.61 μH on LCR meter
...
|-----------|---|---|
| **Inductance** | Loading coil (from Section 2) | 2.37 μH (per arm) |
| **Capacitance** | Variable trimmer, parallel | 7.0 pF (Bourns 3296) |
| **Resonant frequency** | Design center | 21.225 MHz |
| **Trap inductance** | Calculated from LC resonance | 8.032 μH |
...
**Verification:** 8.032 μH || 7.0 pF should resonate at 21.225 MHz ± 1%
...
1. **Initial adjustment (approximate):**
   ...
   - Target: Minimum dip at 21.225 MHz
...
| **Radiator wire notches** | 1/16" dia holes, 42 mm spacing |
...
3. **Radiator wire routing:**
   - Total wire length: 3.36 m (11.0 feet)
...
**Required equipment:**
- SWR/power meter (capable of 21.00–21.45 MHz)
...
3. **Trimmer adjustment (dip meter preferred):**
   ...
   - Dip frequency should be 21.225 MHz ± 0.5%
   - Re-check SWR; should drop to < 1.5:1

4. **Band-edge SWR verification:**
   - Record SWR at band low edge: __________ @ 21.000 MHz
   - Record SWR at band center: __________ @ 21.225 MHz (target < 1.5:1)
   - Record SWR at band high edge: __________ @ 21.450 MHz
   - All edges should be < 2.5:1; center < 1.5:1

### 5.2 NEC2 Simulation

Load model file: `compact_dipole_15m.nec`
...
6. Adjust trimmer with dip meter for minimum dip at 21.225 MHz
...
The following diagrams are located in this band folder (`15m/figures/`):

1. **15m_antenna_layout.png** — Scale deployment drawing with dimensions
2. **15m_coil_detail.png** — Loading coil cross-section, winding detail
3. **15m_trap_schematic.png** — Trap resonator circuit, L/C values
