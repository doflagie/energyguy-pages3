...
**ELOs:** Calibrate the TRANSMATCH — PORTABLE ANTENNA TUNER; verify performance against acceptance criteria; complete calibration log
...
Open TM-GEAR-018. Read **Chapter 6 — Calibration** and **Chapter 7 — Verification and Acceptance** completely.
...
1. Verify SWR indicator zero: connect 50Ω load to the antenna port.
 Transmit 10W. Reflected LED must not illuminate. Adjust SWR bridge
 null trimmer if reflected shows any indication.
2. Verify SWR indicator response: connect a 100Ω load (SWR 2:1).
 Reflected LED must illuminate at approximately 30% of the forward LED
 intensity (indicating ~2:1 SWR).
...
2. Matches a 450Ω random wire (via 9:1 un-un) to SWR <2:1 on
 all HF bands.
...
6. Log: date, match test (200Ω at 7 and 14 MHz, SWR achieved),
 insertion loss check, VHF section test, operator.
...
A criterion found in Chapter 6: **30%**. Confirm the exact criterion in the TM.
