...
1. Drive two 8′ ground rods at least 16′ apart (one rod length spacing
 minimum per NEC 250). Rods must be buried full depth (8′/2.4 m).
2. Bond rods together with #6 AWG bare copper, buried 300 mm below grade.
 Use irreversible compression connectors (Burndy or equivalent) — never
 rely on wire wrapped around the rod.
3. Run the ground conductor from the rod cluster to the entry panel via the
 shortest possible path (4-2 Entry Panel

1. Mount the copper panel on the exterior wall at the point where all antenna
 cables enter the shack. Mount as close to the ground rod as possible.
2. All coaxial feedlines must pass through the panel via GDT-equipped
 bulkhead connectors. All cable shields must bond directly to the panel
 at this point.
3. Bond the panel to the ground rod cluster with a minimum 1/2&Prime; copper
 strap (no wire — strap has lower RF impedance).
