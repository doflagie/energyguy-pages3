...
**ELOs:** Understand the operating principle of the END-FED HALF-WAVE (EFHW) ANTENNA; identify key electrical characteristics
...
Open TM-ANT-019. Read **Chapter 2 — Theory of Operation** completely.
...
86-ft wire resonant at 3.5 mhz; fed at high-impedance end via 49:1 unun transformer. An end-fed wire presents a high impedance at its end: ~2500–5000 Ω for a half-wave wire; ~1000–2000 Ω for a quarter-wave wire. This high impedance is transformed down to 50 Ω by the matching transformer or unun. Design values: 2500–5000 Ω at wire end → 50 Ω via transformer. The transformation ratio n² = Zwire/Zcoax; for a 49:1 unun, n = 7, so the wire impedance appears as 49×50 = 2450 Ω from the wire side.

### 2-2. RADIATION PATTERN

An end-fed wire radiates in a pattern that varies with length and frequency. A half-wave wire at 10 ft height produces a bidirectional broadside pattern similar to a dipole. At harmonic frequencies the pattern has multiple lobes and nulls. Gain per design: 2.1 dBi broadside. The pattern is affected by the run of the feed line if common-mode current is not controlled.
...
**SC1-1.** In one sentence, state the operating principle of the END-FED HALF-WAVE (EFHW) ANTENNA as described in Chapter 2.

**SC1-2.** What determines the resonant frequency of the END-FED HALF-WAVE (EFHW) ANTENNA? Name the primary physical parameter(s).

**SC1-3.** What feedpoint impedance does Chapter 2 predict for the END-FED HALF-WAVE (EFHW) ANTENNA in free space? How does that change over real ground?

**SC1-4.** What radiation pattern does the END-FED HALF-WAVE (EFHW) ANTENNA produce? What are the nulls and maxima directions?
...
- The operating principle of the END-FED HALF-WAVE (EFHW) ANTENNA
