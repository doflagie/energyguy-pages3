...
Open TM-ANT-035. Read **Chapter 3 — Materials and Construction** completely.
...
| 1 | Feed connector | SO-239 or N-type; 50 Ω (via λ/4 matching strip) system |
| 1 | Support hardware | UV-stable; sized for operating environment (wind, ice load) |
| As needed | Feed line | RG-213 or LMR-400; match to feed impedance 50 Ω (via λ/4 matching strip) |
...
All critical dimensions are embedded in the NEC2 model wire card (GW) definitions. Extract from helical_antenna_2m_satellite.nec for precise construction reference.
