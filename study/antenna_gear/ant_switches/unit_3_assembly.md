...
Open TM-GEAR-001. Read **Chapter 4 — Construction and Assembly** completely.
...
1. Mount SO-239 connectors on the enclosure with 4-2 Relay Switch

1. Mount relays in a grid pattern with <15 mm of coax between each relay
 and its associated SO-239. Longer stubs degrade VHF performance.
2. Bypass each relay coil with a 1N4007 flyback diode (cathode to +12V)
 to suppress the inductive spike when the relay de-energizes.
3. Mount ESP32 on a separate PCB with 10 nF bypass caps on each GPIO pin
